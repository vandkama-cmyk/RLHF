...
class EnhancedLLMClassifier(nn.Module):
    ...
    def __init__(
        ...
        try:
            ...
            self.tokenizer = AutoTokenizer.from_pretrained(model_path, token=None)
            self.config = AutoConfig.from_pretrained(model_path, token=None)
            self.encoder = AutoModel.from_pretrained(model_path, token=None)
