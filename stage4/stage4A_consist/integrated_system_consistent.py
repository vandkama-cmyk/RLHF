...
MODIFIED FOR CONSISTENT METRIC ONLY

Core MLP classifier with anti-overfitting techniques for code quality assessment.
Uses 'consistent' metric from human feedback instead of 'useful'.
"""
...
def iter_feedback_samples(feedback_dir: Path, dataset_dir: Optional[Path] = None) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        # Resolve feedback_dir: strip any old prefix so we always join from cwd/stage4/
...
class IntegratedTrainingPipeline:
    ...
    def train(self, train_loader: DataLoader, val_loader: DataLoader, num_epochs: int = 30) -> Dict[str, Any]:
        ...
        print("=== ClassifMLP Training (CONSISTENT METRIC) ===")
        ...
        for epoch in range(1, num_epochs + 1):
            ...
            train_consistent_acc = train_acc
            val_consistent_acc = val_acc
            ...
            history.append({
                ...
                'train_acc': train_acc,
                'val_acc': val_acc,
                'train_consistent_acc': train_consistent_acc,
                'val_consistent_acc': val_consistent_acc,
...
def train_integrated_system(args: argparse.Namespace) -> None:
    """Main training function for ClassifMLP - CONSISTENT METRIC."""
    ...
    print(f"=== ClassifMLP Training (CONSISTENT METRIC) | seed={seed} ===")
    ...
    # MODIFIED: Use 'consistent' as the primary label (instead of 'useful')
    for sample in samples:
        if 'labels' in sample and 'label' not in sample:
            # Use 'consistent' as the primary label for binary classification
            consistent_score = sample['labels']['consistent']

            # Create more balanced binary labels using stricter threshold
            if consistent_score >= 0.8:  # Stricter threshold for positive
                sample['label'] = 1.0
            elif consistent_score <= 0.4:  # Only very low scores as negative
                sample['label'] = 0.0
            else:
    ...
    print("DEBUG: Sample data inspection (CONSISTENT METRIC):")
    ...
    history_path = output_dir / "training_history_consistent.json"
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="ClassifMLP - Neural Code Quality Classifier (CONSISTENT METRIC)")

    parser.add_argument("--device", type=str, default="cuda" if torch.cuda.is_available() else "cpu")
    parser.add_argument("--feedback-dir", type=str, default="evaluation_results_server")
    parser.add_argument("--output-dir", type=str, default="stage4/stage4A_consist/artifacts")
