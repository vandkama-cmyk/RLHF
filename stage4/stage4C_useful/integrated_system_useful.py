...
# Project root (…/Version_1)
_PROJECT_ROOT = Path(__file__).resolve().parent.parent.parent


# =============================================================================
# FOCAL LOSS (binary) — mitigates collapse on imbalanced usefulness labels
# =============================================================================


class FocalBCEWithLogitsLoss(nn.Module):
    """Focal loss over BCEWithLogits for hard-example emphasis (Lin et al., 2017)."""

    def __init__(self, gamma: float = 2.0, pos_weight: Optional[torch.Tensor] = None):
        super().__init__()
        self.gamma = gamma
        self.register_buffer("pos_weight_buf", pos_weight if pos_weight is not None else torch.tensor(1.0))

    def forward(self, logits: torch.Tensor, targets: torch.Tensor) -> torch.Tensor:
        pw = self.pos_weight_buf
        if pw.device != logits.device:
            pw = pw.to(logits.device)
        prob = torch.sigmoid(logits)
        p_t = prob * targets + (1.0 - prob) * (1.0 - targets)
        bce = F.binary_cross_entropy_with_logits(logits, targets, reduction="none", pos_weight=pw)
        mod = (1.0 - p_t).clamp(min=1e-6).pow(self.gamma)
        return (mod * bce).mean()
...
class IntegratedTrainingPipeline:
    ...
    def train(self, train_loader: DataLoader, val_loader: DataLoader, num_epochs: int = 30) -> Dict[str, Any]:
        ...
        print("=== ClassifMLP Training (USEFUL METRIC) ===")
        ...
        for epoch in range(1, num_epochs + 1):
            ...
            for batch in tqdm(train_loader, desc="Training"):
                ...
                if self.debug_logging and epoch == 1 and len(extended_features) > 0:
                    logger.debug("Feature tensor shape: %s", extended_features.shape)
                    logger.debug("Expected input dim: %s", self.classifier.net[0].in_features)
                ...
                if self.debug_logging and epoch == 1 and len(questions) > 0:
                    pred_vals = torch.sigmoid(logits).detach().cpu().numpy()
                    label_vals = labels.detach().cpu().numpy()
                    logger.debug(
                        "Train batch preds mean=%.3f std=%.3f | labels mean=%.3f std=%.3f | acc=%.3f",
                        pred_vals.mean(),
                        pred_vals.std(),
                        label_vals.mean(),
                        label_vals.std(),
                        (predictions == labels.long()).float().mean().item(),
                    )
            ...
            train_useful_acc = train_acc
            val_useful_acc = val_acc
...
def train_integrated_system(args: argparse.Namespace) -> None:
    """Main training function for ClassifMLP - USEFUL METRIC."""
    ...
    print(f"=== ClassifMLP Training (USEFUL METRIC) | seed={seed} ===")
    ...
    # Use 'useful' as the primary label
    ...
    print("DEBUG: Sample data inspection (USEFUL METRIC):")
    ...
    history_path = output_dir / "training_history_useful.json"
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="ClassifMLP - Neural Code Quality Classifier (USEFUL METRIC)")

    parser.add_argument("--device", type=str, default="cuda" if torch.cuda.is_available() else "cpu")
    parser.add_argument("--feedback-dir", type=str, default="evaluation_results_server")
    parser.add_argument("--output-dir", type=str, default="stage4/stage4C_useful/artifacts")
